...
//identifier le nom de base de données
    ...
    if ($db_found) {
            $sql = "SELECT * FROM 'coach' WHERE 'email'=$login ";
            $result = mysqli_query($db_handle, $sql);
                    if($data['mdp']== $mdp ){
                        $_SESSION["ID-coach"] =$data['ID-coach'];
                        session_start();

                     header('Location:profil-coach.html'); // connexion reussi chargement de la page suivante
                    }
                    else 
